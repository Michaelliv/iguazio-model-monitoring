...
from model_monitoring.alert_log import TSDBAlertLog
...
from model_monitoring.endpoint import EndpointKey, EndpointState
...
class EventStreamProcessor:
    ...
    def handle_event(self, event):
        ...
        event_feature_cols = event["feature_col"]
        event_inputs = event_outputs["inputs"]
        event_predictions = event_outputs["prediction"]

        # Handle individual predictions
        for features, prediction in zip(event_inputs, event_predictions):
            self.handle_prediction(endpoint_key, endpoint_state, event, features, event_feature_cols, prediction)

    def handle_prediction(self, endpoint_key, endpoint_state, event, features, feature_cols, prediction):
        ...
        prediction_dict = asdict(endpoint_key)
        prediction_dict["features"] = compute_named_features(feature_cols, features)
        prediction_dict["prediction"] = prediction
        prediction_dict["microsec"] = event["microsec"]
        prediction_dict["timestamp"] = timestamp

        endpoint_state.predictions.append(prediction_dict)
